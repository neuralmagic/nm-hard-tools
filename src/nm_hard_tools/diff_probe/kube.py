"""Small Kubernetes boundary for probe Jobs, easy to fake in contract tests."""
...
from nm_hard_tools.diff_probe.planner import ANNOTATION_PREFIX, ID_LABEL

CANCELLED_AT_ANNOTATION = f"{ANNOTATION_PREFIX}cancelled-at"
...
class KubernetesClient:
    ...
    def cancel_job(
        ...
        body = {
            "metadata": {
                ...
                "annotations": {CANCELLED_AT_ANNOTATION: cancelled_at},
            },
            "spec": {"suspend": True},
        }
    ...
    def wait_for_job_quiescence(
        ...
        selector = f"{ID_LABEL}={probe_id}"
        while True:
            job = self.get_job(namespace, probe_id)
            ...
            if time.monotonic() >= deadline:
                raise TimeoutError(f"worker for {probe_id} is still terminating")
            time.sleep(0.25)

    def read_logs(self, namespace: str, probe_id: str, tail_lines: int) -> str:
        pods = self._core.list_namespaced_pod(
            namespace,
            label_selector=f"{ID_LABEL}={probe_id}",
            limit=1,
        ).items
        if not pods:
            return ""
